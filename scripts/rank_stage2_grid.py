...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Rank Stage 2 EBIE grid-search outputs.",
    )
    parser.add_argument(
        "outputs_dir",
        help="Directory containing the Stage 2 output JSON files.",
    )
    parser.add_argument(
        "--top",
        type=int,
    ...
    parser.add_argument(
        "--save-json",
        action="store_true",
        help="Save ranking_stage2_grid.json and best_stage2_grid.json in outputs_dir.",
    )
...
def main():
    ...
    for index, row in enumerate(ranked[: args.top], start=1):
        print(
            ...
            f"pmut={row['prob_mutacao_embedding']} "
            f"mint={row['mutation_intensity_percent']} "
            f"pcross={row['prob_crossover_embedding']} "
            f"padd={row['prob_add_random_token']} "
            f"prem={row['prob_remover_token']} "
