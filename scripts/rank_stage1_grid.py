...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Rank Stage 1 EBIE grid-search outputs.",
    )
    parser.add_argument(
        "outputs_dir",
        help="Directory containing the Stage 1 output JSON files.",
    )
    parser.add_argument(
        "--top",
        type=int,
...
def main():
    ...
    for index, row in enumerate(ranked[: args.top], start=1):
        print(
            ...
            f"best_fitness_mean={row['best_fitness_mean']:.9f} "
            f"std={row['best_fitness_std']:.9f}"
        )
