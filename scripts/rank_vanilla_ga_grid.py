...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Rank Vanilla GA grid-search outputs.",
    )
    parser.add_argument(
        "outputs_dir",
        help="Directory containing the Vanilla GA output JSON files.",
    )
    parser.add_argument(
        "--top",
        type=int,
        default=10,
        help="Number of ranked configurations to print.",
    )
    parser.add_argument(
        "--save-json",
        action="store_true",
        help="Save ranking_vanilla_ga_grid.json and best_vanilla_ga_grid.json in outputs_dir.",
    )
...
def load_rows(outputs_dir):
    rows = []
    pattern = "historico_completo_vanilla_ga_*_vanilla_ga_grid_*.json"
        ...
        rows.append(
            {
                ...
                "ga_mutation_prob": config.get(
                    "ga_mutation_prob",
                    config["prob_mutacao_embedding"],
                ),
                "ga_crossover_prob": config.get(
                    "ga_crossover_prob",
                    config["prob_crossover_embedding"],
                ),
                "ga_embedding_mutation_std": config.get("ga_embedding_mutation_std"),
...
def main():
    ...
    if not rows:
        raise SystemExit(f"Nenhum arquivo de resultado encontrado em {outputs_dir}")

    ranked = sorted(rows, key=build_sort_key)
    if args.save_json:
        save_json(outputs_dir / "ranking_vanilla_ga_grid.json", ranked)
        save_json(outputs_dir / "best_vanilla_ga_grid.json", ranked[0])
    ...
    for index, row in enumerate(ranked[: args.top], start=1):
        print(
            ...
            f"pop={row['populacao_inicial']} gen={row['num_geracoes']} "
            f"ga_pmut={row['ga_mutation_prob']} "
            f"ga_pcross={row['ga_crossover_prob']} "
            f"ga_sigma={row['ga_embedding_mutation_std']} "
            f"tournament={row['tournament_size']} "
            f"success_rate={row['success_rate']} "
            f"evals_to_target={row['evaluations_to_target_mean']} "
            f"best_fitness_mean={format_metric(row['best_fitness_mean'])} "
            f"std={format_metric(row['best_fitness_std'])} "
            f"avg_len={format_metric(row['average_length_final_mean'], 3)} "
            f"lexical_diversity={format_metric(row['lexical_diversity_mean'], 3)}"
        )
