...
def hill_climbing(resources, config, solucao_inicial):
    ...
    score_atual = solucao_details["objective_value"]
    ...
    while logger.evaluation_counter < max_evaluations:
        ...
        for _ in range(max_neighbors):
            variation_result = gerar_variacao(
                resources,
                config,
                solucao_atual,
        ...
        vizinhos_details = evaluate_and_log_decoded_embeddings(
            logger,
            generation=geracao,
            texts=vizinhos_textos,
            embeddings=vizinhos_embeddings,
        ...
        for vizinho_detail in vizinhos_details:
            ...
            if vizinho_detail["objective_value"] > melhor_score:
                melhor_vizinho = vizinho_detail["decoded_text"]
                melhor_score = vizinho_detail["objective_value"]
        ...
        if melhor_score > score_atual:
            ...
            restart_detail = evaluate_and_log_texts(
                logger,
                generation=geracao,
                texts=[restart_solution],
                operator_records=[
                    {
                        "parent_ids": [candidate_id_atual],
                        "parent1_id": candidate_id_atual,
                        "parent2_id": None,
                        "operator_used": "restart",
                        "mutation_type": None,
                        "crossover_type": None,
