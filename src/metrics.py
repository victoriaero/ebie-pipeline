...
def get_target_label_id(resources, config):
    if config.get("classifier_target_class") is not None:
        target_class = str(config["classifier_target_class"]).strip().lower()
        id2label = getattr(resources.classifier_model.config, "id2label", {}) or {}
...
def _get_target_class(resources, config):
    ...
    return _get_label_name(resources, get_target_label_id(resources, config))
...
def _get_evaluation_budget(config, population_size):
    ...
    if method == "cma_es":
        if config.get("cma_es_classifier_evals") is not None:
            return config["cma_es_classifier_evals"]
...
def _classify_texts(resources, config, texts):
    target_label_id = get_target_label_id(resources, config)
...
class RunLogger:
    def __init__(self, resources, config, population_size, initial_population):
        ...
        self.target_label_id = get_target_label_id(resources, config)
    ...
    def flush_embeddings(self):
        ...
        if not self.embedding_rows:
            embeddings_flat = np.empty((0,), dtype=np.float32)
...
def summarize_runs(run_summaries):
    ...
    valid_evals = [
        run["evaluations_to_target"]
        for run in run_summaries
        if run["evaluations_to_target"] is not None
    ]
